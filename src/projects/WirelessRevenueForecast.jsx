...
function ScenarioTab() {
  ...
  const scenarioLabels = { bear: "Bear", base: "Base", bull: "Bull" };
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16, flexWrap: "wrap" }}>
            ...
            return (
              <button
                ...
                style={{
                  padding: "5px 16px",
                  borderRadius: 20,
                  border: `1.5px solid ${active ? C.VZ : C.grid}`,
                  background: active ? C.VZ : "#fff",
        ...
        <span style={{
          fontSize: 11, color: "#16a34a", fontFamily: "'Outfit', sans-serif",
      ...
      <div style={{ background: "#fff", borderRadius: 10, padding: "24px 20px", boxShadow: "0 1px 4px rgba(11,22,40,0.07)", marginBottom: 20 }}>
        ...
        <ResponsiveContainer width="100%" height={340}>
          <ComposedChart data={chartData} margin={{ top: 8, right: 24, bottom: 4, left: 8 }}>
            ...
            <Line dataKey="actual" name="Historical" stroke={C.VZ} strokeWidth={1.5} strokeDasharray="3 2" dot={false} connectNulls />
            <Line dataKey="bull_v" name="Bull" stroke={C.VZ} strokeWidth={scenario === "bull" ? 3 : 1.8} strokeDasharray={scenario === "bull" ? undefined : "5 3"} dot={false} connectNulls opacity={scenario === "bull" ? 1 : 0.6} />
            <Line dataKey="base_v" name="Base" stroke={C.VZ} strokeWidth={scenario === "base" ? 3 : 2} dot={false} connectNulls opacity={scenario === "base" ? 1 : 0.5} />
            <Line dataKey="bear_v" name="Bear" stroke={C.VZ} strokeWidth={scenario === "bear" ? 3 : 1.8} strokeDasharray={scenario === "bear" ? undefined : "5 3"} dot={false} connectNulls opacity={scenario === "bear" ? 1 : 0.45} />
          </ComposedChart>
        </ResponsiveContainer>
      </div>
            ...
            <tbody>
              {[
                ...
                <tr key={label} style={{ background: ri % 2 === 0 ? "#F4F5F8" : "#fff" }}>
                  ...
                  <td style={{ padding: "9px 14px", textAlign: "center", color: scenario === "bear" ? C.VZ : C.tick, fontWeight: scenario === "bear" ? 700 : 400 }}>{fmt(bear_v)}</td>
                  <td style={{ padding: "9px 14px", textAlign: "center", color: scenario === "base" ? C.VZ : C.navy, fontWeight: scenario === "base" ? 700 : 600 }}>{fmt(base_v)}</td>
                  <td style={{ padding: "9px 14px", textAlign: "center", color: scenario === "bull" ? C.VZ : C.tick, fontWeight: scenario === "bull" ? 700 : 400 }}>{fmt(bull_v)}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
function SensitivityTab() {
  ...
  function computeRevForDrivers(netAdds, arpuGrowth, churn) {
    const forecast = buildForecast(START.subs, START.arpu, netAdds, arpuGrowth, churn, sc.churn, QUARTERS);
    return { fy25: +getFY(forecast, 2026).toFixed(3), fy26: +getFY(forecast, 2027).toFixed(3) };
  }

  const baseResult = computeRevForDrivers(sc.netAdds, sc.arpuGrowth, sc.churn);

  const drivers = [
    { label: "Net Adds",    key: "netAdds",    getFn: (pct) => computeRevForDrivers(sc.netAdds * (1 + pct / 100), sc.arpuGrowth, sc.churn) },
    { label: "ARPU Growth", key: "arpuGrowth", getFn: (pct) => computeRevForDrivers(sc.netAdds, sc.arpuGrowth * (1 + pct / 100), sc.churn) },
    { label: "Churn Rate",  key: "churn",      getFn: (pct) => computeRevForDrivers(sc.netAdds, sc.arpuGrowth, sc.churn * (1 + pct / 100)) },
  ];
  ...
  drivers.forEach(d => {
    [-10, -5, 5, 10].forEach(pct => {
      ...
      tornadoBars.push({ label: `${d.label} ${pct > 0 ? "+" : ""}${pct}%`, delta, color: delta >= 0 ? "#22c55e" : "#ef4444", driver: d.label, pct, fy25: res.fy25, fy26: res.fy26 });
    });
  });
      ...
      <div style={{ background: "#fff", borderRadius: 10, padding: "24px 20px", boxShadow: "0 1px 4px rgba(11,22,40,0.07)", marginBottom: 20 }}>
        ...
        <ResponsiveContainer width="100%" height={300}>
          <BarChart layout="vertical" data={tornadoBars} margin={{ top: 4, right: 60, bottom: 4, left: 120 }}>
            ...
            <XAxis type="number" tick={axisStyle} tickFormatter={v => (v >= 0 ? "+" : "") + v.toFixed(2) + "B"} domain={["auto", "auto"]} />
            <YAxis type="category" dataKey="label" tick={{ fontSize: 11, fill: C.tick }} width={118} />
            <Tooltip formatter={(v) => [`${v >= 0 ? "+" : ""}$${v.toFixed(3)}B`, "Delta vs Base"]} labelStyle={{ fontFamily: "'Outfit', sans-serif", fontSize: 12 }} />
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, fontFamily: "'Outfit', sans-serif" }}>
                <thead>
                  <tr>
                    {["Variation", "Driver Value", "FY2026E Rev ($B)", "FY2027E Rev ($B)", "vs Base ($B)", "vs Base (%)"].map((h, i) => (
                      <th key={h} style={{ padding: "8px 12px", textAlign: i === 0 ? "left" : "center", background: "#F8F9FF", color: C.navy, fontWeight: 600, borderBottom: `2px solid ${C.VZ}`, whiteSpace: "nowrap" }}>
                        {h}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {tableRows.map(({ pct, label }) => {
                    ...
                    const driverVal = isBase ? sc[d.key] : sc[d.key] * (1 + pct / 100);
                    ...
                    return (
                      <tr key={pct} style={{ background: rowBg }}>
                        ...
                        <td style={{ padding: "8px 12px", textAlign: "center", fontWeight: isBase ? 700 : 400, color: C.navy }}>${res.fy25.toFixed(2)}B</td>
                        <td style={{ padding: "8px 12px", textAlign: "center", color: C.tick }}>${res.fy26.toFixed(2)}B</td>
...
function CustomTab() {
  const { setScenario, customDrivers, setCustomDrivers } = useWireless();
  ...
  const reset = () => {
    setCustomDrivers({ netAdds: 100, arpuGrowth: 0.35, churn: 0.92 });
    setScenario("custom");
  };

  const customForecast = buildForecast(
    START.subs, START.arpu,
    customDrivers.netAdds, customDrivers.arpuGrowth, customDrivers.churn,
    SCENARIOS.base.churn, QUARTERS
  );
      ...
      <div style={{ flex: "0 0 300px", minWidth: 260 }}>
        <div style={{ background: "#fff", borderRadius: 10, boxShadow: "0 1px 4px rgba(11,22,40,0.07)", overflow: "hidden" }}>
          <div style={{ background: C.VZ, padding: "12px 16px" }}>
            <span style={{ color: "#fff", fontWeight: 700, fontSize: 15, fontFamily: "'Outfit', sans-serif" }}>Verizon</span>
          </div>
          <div style={{ padding: "18px 16px" }}>
            {Object.entries(SLIDER_CONFIG).map(([key, cfg]) => {
              const val = customDrivers[key];
              const displayVal = key === "netAdds" ? `${val > 0 ? "+" : ""}${val}K` : `${val.toFixed(2)}%`;
              return (
                <div key={key} style={{ marginBottom: 18 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 4 }}>
                    ...
                    <span style={{ fontSize: 15, fontWeight: 700, color: C.VZ, fontFamily: "'Outfit', sans-serif" }}>{displayVal}</span>
                  </div>
                  <input
            ...
            <button onClick={reset} style={{ width: "100%", padding: "8px", borderRadius: 6, border: `1px solid ${C.VZ}`, background: "transparent", color: C.VZ, fontSize: 12, fontFamily: "'Outfit', sans-serif", cursor: "pointer", fontWeight: 600, marginTop: 4 }}>
              Reset to Base
            </button>
            <div style={{ marginTop: 12, padding: "8px 10px", background: "#F0FDF4", borderRadius: 6, border: "1px solid #bbf7d0" }}>
              <span style={{ fontSize: 11, color: "#16a34a", fontFamily: "'Outfit', sans-serif" }}>
                ⟳ Board Report is synced to this custom scenario
              </span>
            </div>
          </div>
        </div>
        ...
        <div style={{ background: "#fff", borderRadius: 10, padding: "20px", boxShadow: "0 1px 4px rgba(11,22,40,0.07)", marginBottom: 16 }}>
          ...
          <ResponsiveContainer width="100%" height={280}>
            <LineChart data={chartData} margin={{ top: 4, right: 20, bottom: 4, left: 8 }}>
              ...
              <Tooltip formatter={(v, name) => [fmtRev(v), name]} />
              <Line dataKey="custom" name="Custom" stroke={C.VZ} strokeWidth={2.2} dot={false} />
              <Line dataKey="base" name="Base" stroke={C.VZ} strokeWidth={1.4} strokeDasharray="4 2" dot={false} opacity={0.6} />
            </LineChart>
          </ResponsiveContainer>
        </div>

        <div style={{ background: "#fff", borderRadius: 10, boxShadow: "0 1px 4px rgba(11,22,40,0.07)", overflow: "hidden", borderTop: `3px solid ${C.VZ}` }}>
...
export default function WirelessRevenueForecast() {
  ...
  const { setScenario } = useWireless();

  const handleTabChange = (tabId) => {
    setActiveTab(tabId);
    if (tabId === "custom") setScenario("custom");
  };
      ...
      <div style={{ background: C.navy, borderRadius: 12, padding: "28px 28px 24px", marginBottom: 28 }}>
        ...
        <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
          {[
            ...
            <div key={card.label} style={{ background: "rgba(255,255,255,0.07)", borderRadius: 8, padding: "14px 18px", flex: "1 1 160px", minWidth: 140 }}>
              <div style={{ fontSize: 11, color: C.muted, fontFamily: "'Outfit', sans-serif", marginBottom: 4 }}>{card.label}</div>
              <div style={{ fontSize: 22, fontWeight: 700, color: "#fff", fontFamily: "'Cormorant Garamond', serif", lineHeight: 1.2 }}>{card.value}</div>
              <div style={{ fontSize: 11, color: C.muted, fontFamily: "'Outfit', sans-serif", marginTop: 3 }}>{card.sub}</div>
            </div>
          ))}
        </div>
      </div>
